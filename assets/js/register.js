...
function checkAge(doby,dobm,dobd) {
    var today = new Date();
    var birthdate = new Date(doby,dobm,dobd);
...
function checkUser(username,email,callback) {
	...
	var XHR=createXHR();
	XHR.open("POST","/theatresite/php/checkUser.php",false);
...
function checkForm(e) {
	...
	with(theform) {
		...
		if(checkEmpty(username.value)) 
			{ usernameFb.innerHTML="Please enter a username"; iscomplete=false; testuser=false; }
		else
		...
		if(checkEmpty(secondpass.value)) 
			{ secondpassFb.innerHTML="Please confirm your password"; iscomplete=false;}
		else
